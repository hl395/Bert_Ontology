...
# task specific parameter( multi-label analysis)
flags.DEFINE_integer("num_classes", 28, "Total number of multi-labels for relationship analysis")
...
class MULTILABEL_Processor(DataProcessor):
  """Processor for the MRPC data set (GLUE version)."""
  def get_train_examples(self, data_dir, hier):
    """See base class."""
    file_name = hier + "_"
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, file_name + "train.tsv")), "train")

  def get_dev_examples(self, data_dir, hier):
    """See base class."""
    file_name = hier + "_"
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, file_name + "dev.tsv")), "dev")

  def get_test_examples(self, data_dir, hier):
    """See base class."""
    file_name = hier + "_"
    return self._create_examples(
  ...
  def _create_examples(self, lines, set_type):
    """Creates examples for the training and dev sets."""
    examples = []
    # emot_mapping = {'anger': '0', 'fear': '1', 'joy': '2', 'sadness': '3'}
    for (i, line) in enumerate(lines):
      # print(line)
      if i == 0:
        continue
      ...
      if set_type == "test":
        ...
        label = [x.replace('[', '').replace(']','') for x in label.split()]
        # print(label)
        #label = tokenization.convert_to_unicode(emot_mapping[line[0]])
      examples.append(
          InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
    return examples
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  # print(example.label)
  if isinstance(example.label, list): # multiple label
      label_id = [int(x) for x in example.label]
  else: # single label
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    probabilities = tf.nn.sigmoid(logits)  # changes here
    # log_probs = tf.nn.log_softmax(logits, axis=-1) # changes here
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example):
        # print("###metric_fn.logits:", logits.shape) # (?,6)

        predictions = tf.sigmoid(logits)  # change here.

        def multi_label_hot(prediction, threshold=0.5):
            prediction = tf.cast(prediction, tf.float32)
            threshold = float(threshold)
            return tf.cast(tf.greater(prediction, threshold), tf.int32)

        one_hot_prediction = multi_label_hot(predictions)

        # print("###metric_fn.label_ids:", label_ids.shape, ";predictions:",
        #       one_hot_prediction.shape)  # label_ids: (?,6);predictions:(?,6)

        accuracy = tf.metrics.accuracy(
            labels=label_ids, predictions=tf.math.round(one_hot_prediction), weights=is_real_example)
...
def main(_):
  ...
  if FLAGS.do_train:
    train_examples = processor.get_train_examples(FLAGS.data_dir, FLAGS.hier_name)
    num_train_steps = int(
  ...
  if FLAGS.do_eval:
    eval_examples = processor.get_dev_examples(FLAGS.data_dir, FLAGS.hier_name)
  ...
  if FLAGS.do_predict:
    predict_examples = processor.get_test_examples(FLAGS.data_dir, FLAGS.hier_name)
...
if __name__ == "__main__":
  ...
  FLAGS.data_dir = "glue_data/MULTILABEL/"
  ...
  FLAGS.output_dir = "OUTPUT/procedure/pre_training_multiple_1/"
  FLAGS.init_checkpoint = "MODEL/small/bert_model.ckpt"  # default bert small model
